function generateVoicemailData(recordCount = 10000) {
  ...
  function generateRandomDate() {
    ...
    // Create received date string (RFC format)
    ...
    const dayOfWeek = days[Math.floor(Math.random() * 7)];
  ...
  function generateFileSize(duration: number) {
  ...
  for (let i = 0; i < recordCount; i++) {
    ...
    const filename = `vmail-${currentYear}-${monthName}-${dateInfo.localTime.substring(
      6,
      8
    )}_${dateInfo.displayTime.replace(":", "-")}.wav`;
...
function createVoicemailDump(recordCount = 10000) {
  const records = generateVoicemailData(recordCount);
...
// Usage example:
export const voicemailData = createVoicemailDump(10000);
